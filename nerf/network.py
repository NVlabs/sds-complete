...
def laplace_cumulative(s,beta):
    output = torch.zeros_like(s)
    if (s<=0).any():
        try:
            output[s<=0]=0.5*torch.exp(s[s<=0]/beta)
        except Exception:
            output[s<=0]=0.5*torch.exp(s[s<=0]/beta).half()
    if (s>0).any():
        try:
            output[s>0]=(1-0.5*torch.exp(-s[s>0]/beta))
        except Exception:
            output[s>0]=(1-0.5*torch.exp(-s[s>0]/beta)).half()
    return output
class NeRFNetwork(NeRFRenderer):
    ...
    def common_forward(self, x):
        ...
        sdf= self.sdf_net(x.view(-1,3)).squeeze().view(x.shape[:-1])#(x.norm(dim=-1)-0.5) #SDF to fixed sphere of radius 0.5
        alpha = 0.1*1/0.001
        sigma=alpha*laplace_cumulative(-sdf,0.001)
      
        albedo = torch.sigmoid(h[..., 1:])

        return sigma,sdf, albedo
